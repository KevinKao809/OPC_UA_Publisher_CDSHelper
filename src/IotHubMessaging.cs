...
namespace OpcPublisher
{
    ...
    public class IotHubMessaging
    {
        ...
        public async Task<bool> InitAsync()
        {
            try
            {
                // call CDS Helper, Add by Kevin Kao                      
                CDSHelper cdsHelper = new CDSHelper();
                cdsHelper.Connect().Wait();
                _iotHubOwnerConnectionString = "We Don't Need It";
                _deviceConnectionString = cdsHelper.GetIoTHubDeviceConnectionString();
                _cdsCompanyId = cdsHelper.GetCompanyId();
                _cdsEquipmentId = cdsHelper.GetEquipmentId();
                _cdsMessageCatalogId = cdsHelper.GetMessageCatalogId();
        ...
        private async Task<string> CreateJsonMessage(MessageData messageData)
        {
            try
            {
                ...
                if (!string.IsNullOrEmpty(messageData.ApplicationUri) || !string.IsNullOrEmpty(messageData.DisplayName))
                {
                    ...
                    // process PerformanceCounter; Add by Kevin Kao
                    await _jsonWriter.WritePropertyNameAsync(PerformanceView.cpuPerformance);
                    await _jsonWriter.WriteValueAsync(PerformanceView.GetCPUPerformance());
                    await _jsonWriter.WritePropertyNameAsync(PerformanceView.memoryAvailable);
                    await _jsonWriter.WriteValueAsync(PerformanceView.GetMemeoryAvailable());
                    await _jsonWriter.WritePropertyNameAsync("Gateway");
                    await _jsonWriter.WriteValueAsync(Environment.MachineName.ToLower());
                }
                        ...
                        try
                        {
                            ...
                            if (_iotHubClient != null)
                            {
                                ...
                                try
                                {
                                    ...
                                    /* Add by Kevin Kao for OEE Message */
                                    if (_sentMessages % 100 == 1)
                                    {                                        
                                        string oeePayload = EquipmentOEE.getEquipmentOEEMessage(_cdsCompanyId, _cdsEquipmentId);
                                        var oeeMessage = new Microsoft.Azure.Devices.Client.Message(Encoding.ASCII.GetBytes(oeePayload));
                                        oeeMessage.Properties.Add("MessageCatalogId", _cdsMessageCatalogId);
                                        oeeMessage.Properties.Add("Type", "Message");
                                        await _iotHubClient.SendEventAsync(oeeMessage);                                        
                                    }
                                    /* End of OEE */
        ...
        // Add by Kevin Kao
        private static int _cdsCompanyId;
        private static string _cdsEquipmentId;
        private static string _cdsMessageCatalogId;
    }
}
